...
const int nth =
    10; // for the representation of the fibers in paraview, nb of points -1 per half-circle (end of the fiber)
...
void output_fibres_grains(const string &nomFichier,
                          const string &cheminDestination); // creates the input file for the DEM simulation
...
int main(int argc, char *argv[]) {
  ...
  while (vfibres.size() < nf) // while the nf fibers have not yet been positioned in the box
  {
    ...
    for (i = 0; i < vfibres.size(); i++) // loop on fibers already accepted in the box
    {
      ...
      // main box
      if (doIntersect(f.pos_c_1[0], f.pos_c_1[1], f.pos_c_2[0], f.pos_c_2[1], vfibres[i].pos_c_1[0],
                      vfibres[i].pos_c_1[1], vfibres[i].pos_c_2[0], vfibres[i].pos_c_2[1], f.alpha, vfibres[i].alpha)) {
        // crossing detected
        compteur_f++;
        break;
      }
      ...
      // lower periodic box
      if (doIntersect(f.pos_c_1[0], f.pos_c_1[1] - b_ymax, f.pos_c_2[0], f.pos_c_2[1] - b_ymax, vfibres[i].pos_c_1[0],
                      vfibres[i].pos_c_1[1], vfibres[i].pos_c_2[0], vfibres[i].pos_c_2[1], f.alpha, vfibres[i].alpha)) {
        compteur_f++;
        break;
      }
      ...
      // left periodic box
      if (doIntersect(f.pos_c_1[0] - b_xmax, f.pos_c_1[1], f.pos_c_2[0] - b_xmax, f.pos_c_2[1], vfibres[i].pos_c_1[0],
                      vfibres[i].pos_c_1[1], vfibres[i].pos_c_2[0], vfibres[i].pos_c_2[1], f.alpha, vfibres[i].alpha)) {
        compteur_f++;
        break;
      }
      ...
      // upper periodic box
      milieu_ff1_x = f.pos_c_1[0] - milieu_f_x; // Calculs pour déterminer la distance entre l'extrémité 1 de la fibre
                                                // candidate (f1) et le milieu la fibre f
      milieu_ff1_y = (f.pos_c_1[1] + b_ymax) - milieu_f_y; //
      milieu_ff2_x = f.pos_c_2[0] - milieu_f_x; // Calculs pour déterminer la distance entre l'extrémité 2 de la fibre
                                                // candidate (f2) et le milieu la fibre f
      milieu_ff2_y = (f.pos_c_2[1] + b_ymax) - milieu_f_y; //
  ...
  snprintf(chemin_simu, sizeof(chemin),
           "./Ensemble_echantillons/Rfg_%02d_Ndf_%02d_eta_%02d_Lb_%02d/Echantillon_%d/Simulation", (int)(R_fg * 10),
           (int)Nd_f, (int)(eta * 10), (int)Lb, echantillon_i);
  ...
  cout << "Number of fiber insertion attemps : " << att_f << " " << "Number of grain insertion attempts : " << att_g
       << endl;
  cout << "Surface ratio of the generated system : " << R_fg_calc
       << " ; Compacity of the generated system : " << C_fg_calc << endl;
  ...
  if (outfile.is_open()) {
    ...
    outfile << "Rfg Ns eta Lb nf ng nb_essais_fibres nb_essais_grains nb_essais_total Moy_rayons_grains_calculee "
               "Rfg_calculee Cfg_calculee"
            << endl;
    outfile << R_fg << " " << Nd_f << " " << eta << " " << std::stoi(argv[4]) << " " << nf << " " << ng << " " << att_f
            << " " << att_g << " " << att_f + att_g << " " << R_moy << " " << R_fg_calc << " " << C_fg_calc << endl;
  ...
  // char chemin1[1024];
  // snprintf(chemin1, sizeof(chemin1),
  // "/home/pierre/Bureau/Programmes/Ensemble_echantillons/Rfg_%.2f_Ndf_%d_eta_%.2f_Lb_%d", R_fg, Nd_f, eta, Lb);
...
void output_grains(const string &nomFichier, const string &cheminDestination) {
  ...
  if (fog) {
    ...
    for (i = 0; i < vgrains.size(); i++)
      fog << vgrains[i].x << " " << vgrains[i].y << " 0.0" << endl;
    fog << "POINT_DATA " << ng << endl;
    fog << "VECTORS Radius float" << endl;
    for (i = 0; i < vgrains.size(); i++)
      fog << vgrains[i].R << " 0.0 0.0" << endl;
  }
}
...
bool doIntersect(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, float o1,
                 float o2) {
  ...
  else // tous les autres cas
  {
    ...
    // Dans la suite, on teste si le point d'intersection est situé au niveau des segments où s'il est en en dehors,
    // dans leur prolongement. Pour ce faire, on détermine si x_inter se situe entre les coordonées en x des extrémités
    // des deux segments. Il y a donc plusieurs cas à tester selon l'orientation des fibres
...
void output_fibres_grains(const string &nomFichier, const string &cheminDestination) {
  ...
  char fname2[1024];
  sprintf(fname2, "./Ensemble_echantillons/Rfg_%02d_Ndf_%02d_eta_%02d_Lb_%02d/Echantillon_%d/Simulation",
          (int)(R_fg * 10), (int)Nd_f, (int)(eta * 10), (int)Lb, echantillon_i);
  ...
  if (fog) {
    ...
    fog << "Load IsostaticCompression 4" << endl;
    fog << "fibres " << vfibres.size() << " " << Nd_f << endl;
    for (i = 0; i < vfibres.size(); i++) {
      fog << vfibres[i].pos_c_1[0] << " " << vfibres[i].pos_c_1[1] << " " << vfibres[i].pos_c_2[0] << " "
          << vfibres[i].pos_c_2[1] << " " << vfibres[i].R << endl;
    }
